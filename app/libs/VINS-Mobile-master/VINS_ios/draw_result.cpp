...
DrawResult::DrawResult(float _pitch, float _roll, float _yaw, float _Tx, float _Ty, float _Tz)
        : pitch{_pitch}, roll{_roll}, yaw{_yaw}, Tx{_Tx}, Ty{_Ty}, Tz{_Tz},
          change_view_manualy{false} {
    planeInit = false;
    startInit = false;
    initPoint.setZero();
    Fx = 400;
...
bool checkBorder(const cv::Point2f &pt) {
    ...
    return BORDER_SIZE <= img_x && img_x < HEIGHT - BORDER_SIZE && BORDER_SIZE <= img_y &&
           img_y < WIDTH - BORDER_SIZE;
}
...
float check_scale(const cv::Point2f &pt) {
    ...
    int img_x = cvRound(pt.x);
    int img_y = cvRound(pt.y);
    if (BORDER_SIZE <= img_x && img_x < WIDTH - 10 - BORDER_SIZE && BORDER_SIZE <= img_y &&
        img_y < HEIGHT - 10 - BORDER_SIZE) {
        scale_factor = 1.0;
    } else {
...
vector<Vector3f>
DrawResult::calculate_camera_pose(Vector3f camera_center, Matrix3f Rc, float length) {
...
void DrawResult::drawGround(cv::Mat &result, vector<Vector3f> &point_cloud, Vector3f P_latest,
                            Matrix3f R_latest) {
    Eigen::Matrix3f RIC;
    RIC = Utility::ypr2R(Vector3d(RIC_y, RIC_p, RIC_r)).cast<float>();
    ...
    for (unsigned int i = 0; i < point_cloud.size(); i++) {
        Vector3f Pc;
        Pc = (R_latest * RIC).transpose() *
             (point_cloud[i] - 1.0 * P_latest - R_latest * Vector3f(TIC_X, TIC_Y, TIC_Z));
...
void
DrawResult::drawArrow(cv::Mat &result, Vector3f position, Vector3f direction, Vector3f P_latest,
                      Matrix3f R_latest, bool inAR) {
    Eigen::Matrix3f RIC;
    RIC = Utility::ypr2R(Vector3d(RIC_y, RIC_p, RIC_r)).cast<float>();

    // 计算旋转矩阵
    Vector3f origin_direction = Vector3f(position[0], position[1] + 1, position[2]) - position;
    Vector3f target_direction = direction - position;
//    Matrix3f arrowRotation = target_direction * origin_direction.inverse();
    Vector3f arrowCorners[4] = {
            Vector3f(0.0f, 0.0f, 0.0f), Vector3f(-2.0f, -1.0f, 0.0f), Vector3f(0.0f, 1.0f, 0.0f),
            Vector3f(2.0f, -1.0f, 0.0f)
    };
    Point points[4];
    for (int i = 0; i < 4; i++) {
        arrowCorners[i] = arrowCorners[i] + position;
        Vector3f Pc;
        Pc = (R_latest * RIC).transpose() *
             (arrowCorners[i] - 1.0 * P_latest - R_latest * Vector3f(TIC_X, TIC_Y, TIC_Z));

        cv::Point2f pts;
        pts.x = FOCUS_LENGTH_X * Pc.x() / Pc.z() + PX;
        pts.y = FOCUS_LENGTH_Y * Pc.y() / Pc.z() + PY;

        points[i] = Point(pts.x, pts.y);
    }

    const Point *ppt[1] = {points};
    int npt[] = {4};
    cv::fillPoly(result, ppt, npt, 1, cv::Scalar(200, 0, 0));
}

void DrawResult::drawBox(cv::Mat &result, Vector3f corner_0, Vector3f corner_x, Vector3f corner_y,
                         Vector3f corner_z, float size, Vector3f P_latest, Matrix3f R_latest,
                         bool inAR) {

    Eigen::Matrix3f RIC;
    RIC = Utility::ypr2R(Vector3d(RIC_y, RIC_p, RIC_r)).cast<float>();

    vector<Vector3f> boxCorners;
    boxCorners.push_back(corner_0);
    boxCorners.push_back(corner_x);
    boxCorners.push_back(corner_y);
    boxCorners.push_back(corner_x + corner_y - corner_0);

    boxCorners.push_back(corner_z);
    boxCorners.push_back(corner_x + corner_z - corner_0);
    boxCorners.push_back(corner_y + corner_z - corner_0);
    boxCorners.push_back(corner_x + corner_y + corner_z - 2.0 * corner_0);

    vector<cv::Point2f> boxImage;
    Eigen::Vector3f Pc;
    vector<float> depth_of_coner;
    for (auto it : boxCorners) {
        if (inAR)
            Pc = (R_latest * RIC).transpose() *
                 (it - 1.0 * P_latest - R_latest * Vector3f(TIC_X, TIC_Y, TIC_Z));
        else
...
void DrawResult::drawAR(cv::Mat &result, vector<Vector3f> &point_cloud, Vector3f P_latest,
                        Matrix3f R_latest) {
    cv::Mat aa(HEIGHT, WIDTH, CV_8UC3, Scalar(0, 0, 0));
    result = aa;
    Eigen::Matrix3f RIC;
...
//    drawArrow(result, groundPlanePoint,
//              Vector3f(groundPlanePoint[0], groundPlanePoint[1] + 1, groundPlanePoint[2]), P_latest,
//              R_latest, true);


    ///draw ground area
    if (point_inlier.size() > 28) {
        latestGroundPlanePoint = groundPlanePoint;
    ...
    for (unsigned int i = 0; i < Grounds.size(); i++) {
        if (Grounds[i].boxflag)
            drawBox(result, Grounds[i].ori, Grounds[i].cox, Grounds[i].coy, Grounds[i].coz,
                    Grounds[i].size, P_latest, R_latest, true);
    }
    ...
    if (locationX != locationX_p or locationY != locationY_p) {
        ...
        for (unsigned int i = 0; i < Grounds.size(); i++) {
            Pc = (R_latest * RIC).transpose() *
                 (Grounds[i].center - 1.0 * P_latest - R_latest * Vector3f(TIC_X, TIC_Y, TIC_Z));
        ...
        if (dis_min < 120 and Grounds.size() > 0 and Grounds[dis_min_idx].moveflag and
            finger_state == 1) {
            ...
            rotation_angle = atan2(sqrt((cy - yy) * (cy - yy) + (xx - cx) * (xx - cx)),
                                   FOCUS_LENGTH_X);
            ...
            if (RWP(2, 2) < 0) {
                ...
                Vector3f plane_normal(Grounds[dis_min_idx].initPlane[0],
                                      Grounds[dis_min_idx].initPlane[1],
                                      Grounds[dis_min_idx].initPlane[2]);
                length_direction = abs(
                        -1.0 * (plane_normal.dot(P_latest) + Grounds[dis_min_idx].initPlane[3]) /
                        (plane_normal.dot(unit_direction)));
                direction = unit_direction * length_direction;

                if (length_direction < 20) {
                    Grounds[dis_min_idx].ori = direction + P_latest - (Grounds[dis_min_idx].lix *
                                                                       Grounds[dis_min_idx].size +
                                                                       Grounds[dis_min_idx].liy *
                                                                       Grounds[dis_min_idx].size) /
                                                                      2.0;
                    Grounds[dis_min_idx].cox = Grounds[dis_min_idx].ori +
                                               Grounds[dis_min_idx].lix * Grounds[dis_min_idx].size;
                    Grounds[dis_min_idx].coy = Grounds[dis_min_idx].ori +
                                               Grounds[dis_min_idx].liy * Grounds[dis_min_idx].size;
                    Grounds[dis_min_idx].coz = Grounds[dis_min_idx].ori +
                                               Grounds[dis_min_idx].liz * Grounds[dis_min_idx].size;
                    Grounds[dis_min_idx].center =
                            (Grounds[dis_min_idx].cox + Grounds[dis_min_idx].coy) / 2;
                }
            }
        }
    ...
    ////////////////////////////// scale response
    {
        ...
        if (radiusAR != radius_p) {
            if (dis_min < 120 and finger_state == 3 and Grounds.size() > 0) {
                Grounds[dis_min_idx].size =
                        Grounds[dis_min_idx].size * (1 + (radius_p - radiusAR) * 0.008);

                Grounds[dis_min_idx].ori =
                        (Grounds[dis_min_idx].cox + Grounds[dis_min_idx].coy) / 2 -
                        (Grounds[dis_min_idx].lix * Grounds[dis_min_idx].size +
                         Grounds[dis_min_idx].liy * Grounds[dis_min_idx].size) / 2.0;
                Grounds[dis_min_idx].cox = Grounds[dis_min_idx].ori +
                                           Grounds[dis_min_idx].lix * Grounds[dis_min_idx].size;
                Grounds[dis_min_idx].coy = Grounds[dis_min_idx].ori +
                                           Grounds[dis_min_idx].liy * Grounds[dis_min_idx].size;
    ...
    ///////////////////////////// rotation response
    {
        ...
        if (X0AR != X0_p or Y0AR != Y0_p) {
            if (dis_min < 120 and Grounds.size() > 0) {
                ...
                Grounds[dis_min_idx].lix = (Grounds[dis_min_idx].cox - Grounds[dis_min_idx].ori) /
                                           Grounds[dis_min_idx].size;
                Grounds[dis_min_idx].liy = (Grounds[dis_min_idx].coy - Grounds[dis_min_idx].ori) /
                                           Grounds[dis_min_idx].size;
                Grounds[dis_min_idx].liz = (Grounds[dis_min_idx].coz - Grounds[dis_min_idx].ori) /
                                           Grounds[dis_min_idx].size;
    ...
    if (longPressFlag) {
        ...
        if (dis_min < 120 and Grounds.size() > 0) {
            ...
            Pc = (R_latest * RIC).transpose() * (Grounds[dis_min_idx].coz - 1.0 * P_latest -
                                                 R_latest * Vector3f(TIC_X, TIC_Y, TIC_Z));
    ...
    if (tapFlag and point_inlier.size() > 28) {
        ...
        center_input << xx, yy;
        Pc = (R_latest * RIC).transpose() *
             (groundPlanePoint - 1.0 * P_latest - R_latest * Vector3f(TIC_X, TIC_Y, TIC_Z));
        box_center_xy.x() = FOCUS_LENGTH_X * Pc.x() / Pc.z() + PX;
        box_center_xy.y() = FOCUS_LENGTH_Y * Pc.y() / Pc.z() + PY;
...
void DrawResult::drawBoxVirturCam(cv::Mat &result) {
    ...
    for (unsigned int i = 0; i < Grounds.size(); i++) {
        if (Grounds[i].boxflag) {
            drawBox(result, Grounds[i].ori, Grounds[i].cox, Grounds[i].coy, Grounds[i].coz,
                    Grounds[i].size, camInWorld_T, camInWorld_R, false);
        }
    }
}
...
void
DrawResult::Reprojection(cv::Mat &result, vector<Vector3f> &point_cloud, const Matrix3f *R_window,
                         const Vector3f *T_window, bool box_in_trajectorty) {
    float depth_marker;
